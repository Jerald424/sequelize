...
const Sidebar = ({ children }) => {
  ...
  const navigationAssets = [
    ...
    { name: "Posts", link: "/posts", icon: IoMdChatboxes },
  ];
  ...
  return (
    <div className="sidebar-routes">
      <div onMouseEnter={() => handleHover("on")} onMouseLeave={() => handleHover("off")} className={`total-sidebar ${hover && "sidebar-absolute"} ${toggle || hover ? "total-sidebar-active" : "total-sidebar-inactive"} ${isDark ? "bg-dark" : "bg-light"}`} style={{ boxShadow: `2px 2px 3px ${colors?.popupBg}` }}>
        <div className="sidebar-toggle-part" onClick={handleToggle}>
          ...
          >
            <SlArrowRight size={10} className={!toggle && "toggle-button-off"} />
          </div>
        </div>
        <div className="dfa mt-2" style={{ display: toggle || hover ? "flex" : "none" }}>
        ...
        <div style={{ display: toggle || hover ? "block" : "none" }}>
          <div className="mt-5">
            {navigationAssets?.map((res, i) => (
              <Hover className="dajc p-2 mt-1 br-2 cp" key={i + "navigation"} onClick={() => navigate(res?.link)}>
      ...
      <div className={`total-routes f1 ${toggle ? "total-routes-sidebar-on" : "total-routes-sidebar-off"} pt-3`}>{children}</div>
    </div>
  );
};
